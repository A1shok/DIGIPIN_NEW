(function () {
  ...
  function addressPhrases(context) {
    ...
    function add(phrase) {
      ...
      if (!phrase || phrase.length < 4 || seen[phrase]) return;
      if (/^(?:door|d no|house|flat|floor|line|cross|road|street|\d+[a-z]*)$/.test(phrase)) return;
      seen[phrase] = true;
      out.push(phrase);
    }
    ...
    for (var j = 0; j < words.length; j += 1) {
      if (suffixes[words[j]] && j > 0) {
        add(words[j - 1] + " " + words[j]);
      }
      if (j + 1 < words.length && suffixes[words[j + 1]]) {
        add(words[j] + " " + words[j + 1]);
      }
    }
  ...
  function firstAddressPhrase(context) {
    var pieces = String(context.address1 || "").split(/[,;|]/).map(function (part) {
      return searchable(part);
    }).filter(Boolean);
    var stop = /^(?:door|d no|house|flat|floor|line|cross|road|street|\d+[a-z]*)$/;
    for (var i = 0; i < pieces.length; i += 1) {
      if (pieces[i].length >= 4 && !stop.test(pieces[i])) return pieces[i];
    }
    return "";
  }
  function primaryAddressPhrase(context) {
    return firstAddressPhrase(context) || addressPhrases(context)[0] || searchable(String(context.address1 || "").split(",")[0]);
  }
  function geocodeScore(row, context, pin) {
    var display = searchable(row.display_name || "");
    var compactDisplay = compactSearch(row.display_name || "");
    ...
    var phrases = addressPhrases(context);
    if (phrase && (display.indexOf(phrase) !== -1 || compactDisplay.indexOf(compactPhrase) !== -1)) score += 140;
    for (var p = 0; p < phrases.length; p += 1) {
      if (display.indexOf(phrases[p]) !== -1 || compactDisplay.indexOf(compactSearch(phrases[p])) !== -1) score += 70 - Math.min(40, p * 8);
    }
    for (var i = 0; i < tokens.length; i += 1) {
      if (display.indexOf(tokens[i]) !== -1) score += 18;
    }
    if (pin && display.indexOf(pin) !== -1) score += 18;
  ...
  function geocodeMatchesTypedAddress(row, context, pin, pinEntry) {
    ...
    for (var p = 0; p < phrases.length; p += 1) {
      if (display.indexOf(phrases[p]) !== -1 || compactDisplay.indexOf(compactSearch(phrases[p])) !== -1) {
        return true;
      }
    }
  ...
  function uniqueQueries(context, pin) {
    ...
    var queries = [];
    if (primary) {
      queries.push([primary, context.city, context.state, "India"].filter(Boolean).join(", "));
    }
    for (var i = 0; i < phrases.length; i += 1) {
      queries.push([phrases[i], context.city, context.state, "India"].filter(Boolean).join(", "));
    }
