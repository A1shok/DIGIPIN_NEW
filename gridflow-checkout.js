(function () {
  ...
  function primaryAddressPhrase(context) {
    return addressPhrases(context)[0] || searchable(String(context.address1 || "").split(",")[0]);
  }
  function geocodeScore(row, context, pin) {
    ...
    for (var p = 0; p < phrases.length; p += 1) {
      if (display.indexOf(phrases[p]) !== -1) score += 70 - Math.min(40, p * 8);
    }
  ...
  function geocodeMatchesTypedAddress(row, context, pin) {
    ...
    for (var p = 0; p < phrases.length; p += 1) {
      if (display.indexOf(phrases[p]) !== -1) {
        return true;
      }
    }
  ...
  function uniqueQueries(context, pin) {
    ...
    for (var i = 0; i < phrases.length; i += 1) {
      ...
      queries.push([phrases[i], context.state, "India"].filter(Boolean).join(", "));
    }
    queries = queries.concat([
      ...
      [first, context.state, "India"].filter(Boolean).join(", "),
  ...
  async function geocodeAddress(context, pin) {
    ...
    for (var q = 0; q < queries.length; q += 1) {
      try {
        var res = await fetch("https://nominatim.openstreetmap.org/search?format=json&limit=10&addressdetails=1&countrycodes=in&q=" + encodeURIComponent(queries[q]), {
          cache: "force-cache"
        });
