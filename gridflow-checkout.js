(function () {
  ...
  // CORRECTED & RESTORED CSS STRUCTURAL ARRAY
  var CSS = [
    ...
    ".panel{width:min(94vw,480px);background:#fff;border-radius:8px;padding:20px;box-shadow:0 22px 60px rgba(0,0,0,.24)}",
    ".map{position:relative;width:100%;height:320px;border:1px solid #cbd5e1;border-radius:8px;overflow:hidden;background:#f8fafc;touch-action:none;cursor:crosshair}",
  ...
  function showMap(title, descriptionText, startLat, startLon, done, mountRoot) {
    ...
    modal.innerHTML = [
      "<div class='panel'>",
      "<h2>" + title + "</h2>",
      "<p style='font-size:13px;color:#475569;margin-bottom:12px;line-height:1.4;'>" + descriptionText + "</p>",
      ...
      "<div class='coord'></div>",
      "<div class='row' style='margin-top:12px;display:flex;gap:8px;'><button class='use' style='flex:1;'>Confirm Rooftop Location</button><button class='cancel alt' style='width:auto;padding-left:20px;padding-right:20px;'>Cancel</button></div>",
      "</div>"
    ].join("");
    ...
    loadLeaflet().then(function (L) {
      ...
      // Keep track of the center point as the user drags the map view canvas
      map.on("move", function () {
        var center = map.getCenter();
        point.lat = center.lat;
        point.lon = center.lng;
        qs(modal, ".coord").textContent = "Coordinates: " + point.lat.toFixed(6) + ", " + point.lon.toFixed(6);
      });
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    gps() {
      ...
      navigator.geolocation.getCurrentPosition(function (pos) {
        ...
        // CASE 1: User is away from home address footprint
        if (distanceKM > 5.0) {
          showMap(
        ...
        // CASE 2: User is at home, but device returns rough accuracy (like 114m)
        if (accuracy > 10) {
          showMap(
          ...
          // CASE 3: High-accuracy instant capture
          var code = Core.encode(deviceLat, deviceLon);
          this.save(code, "Rooftop matched flawlessly.");
        }
      }.bind(this), function () {
        // CASE 4: GPS failed or blocked - Fall back instantly to typed area neighborhood map
        showMap(
