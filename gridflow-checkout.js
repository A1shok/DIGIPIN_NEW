(function () {
  ...
  function localizedEntry(entry, db, pin, context) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      ...
      if (!name || haystack.indexOf(name) === -1) continue;
      if (!best || name.length > best.name.length) {
        best = { name: name, row: rows[i] };
      }
    }
  ...
  function geocodeAddress(context, pin) {
    var query = addressQuery(context, pin);
    if (!window.fetch || query.replace(/[^a-z0-9]/gi, "").length < 8) {
      return Promise.resolve(null);
    }

    return fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=in&q=" + encodeURIComponent(query), {
      cache: "force-cache"
    }).then(function (res) {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lon) || !Core.inBounds(lat, lon)) return null;
      return {
        lat: lat,
        lon: lon,
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    async choosePin() {
      ...
      if (!entry.matchedLocality || entry.matchedLocality !== "your phone location") {
