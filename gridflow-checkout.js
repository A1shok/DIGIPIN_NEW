(function () {
  ...
  var CSS = [
    ...
    ".row{display:flex;gap:8px;margin-top:12px}.stack{display:flex;flex-direction:column;gap:8px;margin-top:12px}.freshFlow{display:block}.saved.on+.freshFlow{display:none}",
    "button{border:0;border-radius:8px;padding:12px 14px;min-height:44px;background:#116149;color:#fff;font:700 14px/1.1 inherit;cursor:pointer;transition:transform .16s ease,box-shadow .16s ease,background .16s ease}button:hover{box-shadow:0 8px 20px rgba(17,97,73,.16);transform:translateY(-1px)}",
    ...
    ".saved{display:none;margin-top:14px;align-items:center;gap:12px;border:1px solid #c9ddd5;border-radius:8px;background:#f4fbf8;padding:12px}.saved.on{display:grid;grid-template-columns:1fr auto;}.saved strong{display:block;color:#116149;font-size:14px;line-height:1.2}.saved span{display:block;color:#526071;font-size:12px;line-height:1.35;margin-top:3px}.savedActions{display:flex;align-items:center;gap:10px}.savedActions button{white-space:nowrap}",
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.shadowRoot.innerHTML = [
        ...
        "<div class='saved'><div><strong>Verified delivery point ready</strong><span>Reuse the DIGIPIN confirmed for this address.</span></div><div class='savedActions'><button class='useSaved' type='button'>Use saved</button><button class='newPlace link' type='button'>Change</button></div></div>",
        "<div class='freshFlow'>",
        "<div class='row'><button class='gps' type='button'>Secure 4-Meter Delivery</button><button class='away alt' type='button'>Deliver elsewhere</button></div>",
    ...
    bind() {
      ...
      qs(root, ".useSaved").addEventListener("click", this.useSaved.bind(this));
      qs(root, ".newPlace").addEventListener("click", function () {
        qs(this.shadowRoot, ".saved").classList.remove("on");
        this.showRemote("Verify a new delivery point for this address.");
      }.bind(this));
    ...
    refreshSaved() {
      ...
      if (saved) {
        qs(root, ".saved").classList.add("on");
        qs(root, ".saved span").textContent = "DIGIPIN " + saved.digipin + " is ready for this address.";
      } else {
        qs(root, ".saved").classList.remove("on");
      }
    }
    useSaved() {
