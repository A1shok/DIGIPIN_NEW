(function () {
  ...
  var CSS = [
    ...
    ".map{position:relative;width:100%;aspect-ratio:1;border:1px solid #cbd5e1;border-radius:8px;overflow:hidden;background:#f8fafc;background-image:linear-gradient(90deg,rgba(17,97,73,.15) 1px,transparent 1px),linear-gradient(rgba(17,97,73,.15) 1px,transparent 1px);background-size:10% 10%;touch-action:none}",
    ".cross{position:absolute;left:50%;top:50%;width:38px;height:38px;margin:-19px 0 0 -19px;pointer-events:none}.cross:before,.cross:after{content:'';position:absolute;background:#e11d48}.cross:before{left:18px;top:0;width:2px;height:38px}.cross:after{left:0;top:18px;width:38px;height:2px}",
  ...
  function writeField(field, payload) {
    ...
    var cleaned = current.replace(/\[DIGIPIN:[^\]]+\](?:\s\|\s)?/i, "").trim();
  ...
  function showMap(entry, done) {
    ...
    modal.innerHTML = [
      ...
      "<div class='map'><span class='cross'></span></div><div class='coord'></div>",
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.shadowRoot.innerHTML = [
        ...
        "<div class='stack floor hidden'><input class='floorInput' placeholder='Flat/Floor/Office Number'></div>",
    ...
    async choosePin() {
      ...
      showMap(dbEntryToObject(pin, row), function (point, entry) {
        ...
        if (entry.dense) {
          ...
          this.save(code, "You selected a multi-story building. Please enter Floor & Flat/Office Number to finalize grid delivery.");
          return;
        }
