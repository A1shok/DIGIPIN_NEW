(function () {
  ...
  var CSS = [
    ...
    ".saved{display:none;margin-top:12px;border:1px solid #c9ddd5;border-radius:8px;background:#eef5f2;padding:10px}.saved.on{display:block}.saved p{color:#116149;font-weight:700;margin-bottom:8px}.saved .row{margin-top:0}",
  ...
  function pinMatchesCheckout(entry, context) {
    ...
    if (stateMap[normalized]) {
      var matches = stateMap[normalized];
      for (var k = 0; k < matches.length; k += 1) {
        if (label.indexOf(matches[k]) !== -1) return true;
      }
    }
  ...
  function localizedEntry(entry, db, pin, context) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      if (!Array.isArray(rows[i]) || rows[i].length < 1) continue;
  ...
  function loadLeaflet(root) {
    if (root && !root.querySelector('link[href*="leaflet.css"]')) {
  ...
  function showMap(entry, done, mountRoot) {
    ...
    render();
    createLeafletMap(modal, entry, point).then(function () {
      leafletActive = true;
      render();
    }).catch(function () {
      leafletActive = false;
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.shadowRoot.innerHTML = [
        ...
        "<div class='saved'><p></p><div class='row'><button class='useSaved' type='button'>Use saved DIGIPIN</button><button class='newPlace alt' type='button'>Verify new point</button></div></div>",
    ...
    refreshSaved() {
      ...
      if (saved) {
        ...
        qs(root, ".saved p").textContent = "Verified delivery DIGIPIN found for this address.";
      } else {
    ...
    gps() {
      ...
      navigator.geolocation.getCurrentPosition(function (pos) {
        ...
        if (err && err.code === err.TIMEOUT) {
          this.showRemote("GPS connection timed out. Enter the delivery PIN code.");
        } else {
    ...
    async choosePin() {
      ...
      if (db.v < 2 || !db.localities) {
        setStatus(this.shadowRoot, "Using broad PIN center due to demo database limitations.", "warn");
      }
      if (!pinMatchesCheckout(entry, context)) {
        setStatus(this.shadowRoot, "PIN maps to " + entry.label + ". Verify the location on the map.", "warn");
      }
