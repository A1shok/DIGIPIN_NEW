(function () {
  ...
  // BUG FIX 3 (Sub-Path Correction): Dynamic relative-path mapping for absolute GitHub deployment stability
  function loadWithTimeout(url) {
    ...
    return new Promise(function (resolve) {
      ...
      var timer = setTimeout(function () {
        if (!done) {
          console.warn("GridFlow Warning: Centroid dataset fetch timeout. Falling back to internal cities index.");
          resolve(DEFAULT_DB);
        }
      }, 5000); // Expanded timeout timeline window for production network delivery stability
  ...
  function showMap(entry, done, mountRoot) {
    ...
    modal.innerHTML = [
      ...
      "<p>Click the map or drag the marker directly over your roof to secure pinpoint delivery.</p>",
  ...
  // High-density regional metropolitan polygon evaluator fallback
  function isMetropolitanHighRise(lat, lon) {
    return (Math.abs(lat - 12.9766) < 0.12 && Math.abs(lon - 77.5993) < 0.12) || // Bengaluru
           (Math.abs(lat - 18.9388) < 0.12 && Math.abs(lon - 72.8354) < 0.12);   // Mumbai
  }
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.shadowRoot.innerHTML = [
        ...
        "<div class='stack floor hidden'><input class='floorInput' placeholder='House / Flat / Floor / Landmark (Required for Multi-Story Building)'></div>",
    ...
    bind() {
      ...
      // Mandatory input validation intercept block for flat details
      qs(root, ".floorInput").addEventListener("input", function (ev) {
        var val = ev.target.value.trim();
        if (this.pendingDigipin) {
          if (val.length > 0) {
            this.save(this.pendingDigipin, "Flat/floor verified and locked.", val);
          } else {
            setStatus(this.shadowRoot, "Flat, floor, or suite details cannot be empty for high-density locations.", "err");
          }
        }
      }.bind(this));
    }
    ...
    gps() {
      ...
      navigator.geolocation.getCurrentPosition(function (pos) {
        ...
        var code = Core.encode(c.latitude, c.longitude);
        ...
        // BUG FIX 1 (GPS Integration): Intercept metropolitan high-rise targets 
        if (isMetropolitanHighRise(c.latitude, c.longitude)) {
          this.pendingDigipin = code;
          qs(root, ".floor").classList.remove("hidden");
          qs(root, ".floorInput").setAttribute("required", "true");
          setStatus(root, "High-density footprint matched. Flat, room, or floor value is required to complete pinning.", "warn");
          return;
        }
    ...
    async choosePin() {
      ...
      showMap(dbEntryToObject(pin, row), function (point, entry) {
        ...
        // BUG FIX 1 (Map Selector Integration): Block save and enforce flat number field if dense flag is true
        if (entry.dense || isMetropolitanHighRise(point.lat, point.lon)) {
          this.pendingDigipin = code;
          qs(this.shadowRoot, ".floor").classList.remove("hidden");
          qs(this.shadowRoot, ".floorInput").setAttribute("required", "true");
          this.save(code, "Multi-story sector identified. Flat/floor details must be provided before finalizing metadata submission.", "warn");
          return;
        }
    ...
    save(code, message, floor) {
      var currentFloor = floor || qs(this.shadowRoot, ".floorInput").value.trim();
      
      // Enforce the blocking gate rule state for high-density areas
      if (this.pendingDigipin && !currentFloor) {
        setStatus(this.shadowRoot, "Checkout Blocked: Flat, floor, or room code cannot be blank here.", "err");
        return;
      }

      this.pendingDigipin = ""; // Resets the validation gate lock upon successful save
      var target = inject(this, code, currentFloor);
      this.dispatchEvent(new CustomEvent("gridflow:resolved", { bubbles: true, detail: { digipin: code, target: target } }));
      setStatus(this.shadowRoot, message + " Saved " + code + " into checkout metadata.", message.indexOf("accuracy") >= 0 || message.indexOf("identified") >= 0 || message.indexOf("matched") >= 0 ? "warn" : "ok");
    }
  }
