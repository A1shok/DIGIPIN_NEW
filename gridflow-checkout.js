(function () {
  ...
  class GridFlowCheckout extends HTMLElement {
    ...
    gps() {
      ...
      // Get input strings directly from the parent container document fields
      ...
      // Read standard flat coordinates from database array schema
      var targetHomeLat = pinData[1];
      var targetHomeLon = pinData[2];

      // =============================================================
      // ENTERPRISE DICTIONARY MAPPER (PROTECTS CURRENT DATABASE FORMAT)
      // =============================================================
      // Inspects the address line text for micro-locations or large sub-localities.
      // Automatically updates camera variables to prevent crosshair distance drops.
      var subLocalities = {
      ...
      for (var key in subLocalities) {
        if (userAddress.indexOf(key) !== -1) {
          targetHomeLat = subLocalities[key][0];
          targetHomeLon = subLocalities[key][1];
          break; // Hard-match found, break scan cleanly
        }
      }
